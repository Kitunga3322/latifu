...
import 'package:latifu/Admin/miradi/wajaneinsert.dart';

class wajanelist extends StatefulWidget {
  wajanelist({super.key});
  @override
  State<wajanelist> createState() => _wajanelistState();
}

class _wajanelistState extends State<wajanelist> {
  late List<dynamic> Wajanedata = [];

  Future<void> getdharuraData() async {
    try {
      final response = await http.post(Uri.parse('http://192.168.69.4/CHANGIA/wajanelist.php'));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("WAJANE"), centerTitle: true,
        actions: <Widget>[
          IconButton(onPressed: () {
            Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext context) => const Wajaneinsert(),),);},
