...
import 'package:latifu/Admin/miradi/wagonjwainsert.dart';

class Wagonjwalist extends StatefulWidget {
  const Wagonjwalist({super.key});

  @override
  State<Wagonjwalist> createState() => _WagonjwalistState();
}

class _WagonjwalistState extends State<Wagonjwalist> {
  late List<dynamic> Wagonjwadata = [];

  Future<void> getWagonjwadata() async {
    try {
      final response = await http.post(Uri.parse('http://192.168.69.4/CHANGIA/wagonjwalist.php'));
    ...
    return Scaffold(appBar: AppBar(title: const Text("WAGONJWA"), centerTitle: true,
      actions: <Widget>[
        IconButton(onPressed: () {Navigator.of(context).push(MaterialPageRoute(
          builder: (BuildContext context) => const Wagonjwainsert(),),);},
          icon: const Icon(
          ...
          if (Wagonjwadata.isNotEmpty) {
            return ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                final String imageUrl = 'http://192.168.69.4/CHANGIA/${Wagonjwadata[index]["image"]}'; // Corrected URL
                return Card(
